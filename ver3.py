...
with st.sidebar:
    with st.container(border=True):
        ...
        ouo_api_key = st.text_input("API Key ouo.io", value="8pHuHRq5", type="password", help="Masukkan API Key Anda dari ouo.io.")

    with st.container(border=True):
        st.header("💾 Simpan & Muat Sesi")
        if st.button("Simpan Sesi Saat Ini", use_container_width=True):
            ...
            href = f'<a href="data:file/json;base64,{b64}" download="link_generator_session_{timestamp}.json" style="display:block; text-align:center; background-color: #28a745; color: white; padding: 8px 12px; border-radius: 5px; text-decoration: none;">Unduh File Sesi</a>'
            st.markdown(href, unsafe_allow_html=True)

        json_text_to_load = st.text_area("Tempel konten file .json di sini untuk memuat sesi", height=150)
            ...
            if json_text_to_load:
                try:
                    ...
                    if all(key in loaded_data for key in required_keys):
                        ...
                        st.error("Teks JSON tidak valid.")
                except Exception as e:
                    st.error(f"Gagal memuat: {e}")
            else:
                st.warning("Area teks kosong.")
...
with col1:
    with st.container(border=True):
        ...
        input_mode = st.radio("Mode Input", ["Batch Episode", "Single Link"], horizontal=True, key="input_mode")
        ...
        st.session_state.resolutions = st.multiselect("Pilih Resolusi", options=default_resolutions, default=st.session_state.resolutions)
        
        server_choice = st.selectbox("Pilih Nama Server", options=SERVER_OPTIONS, key="sb_server")
        server_name = st.text_input("Atau Ketik Manual", key="txt_server").strip() if server_choice == SERVER_OPTIONS[0] else server_choice
        
        links_text = st.text_area("Tempel Link (1 per baris)", key="links_text", height=200)

        c1, c2 = st.columns([3, 1])
        if c1.button("+ Tambah Data ke Daftar", type="primary", use_container_width=True):
            ...
            if not all([server_name, links, st.session_state.resolutions]):
                ...
                if len(links) != expected_links:
                    st.error(f"Link tidak sesuai. Diperlukan: {expected_links}, Disediakan: {len(links)}.")
                else:
                    link_idx = 0
                    ...
                    for ep in episode_range:
                        ...
                        for res in st.session_state.resolutions:
                            ...
                            st.session_state.main_data[ep][res][server_name] = links[link_idx]; link_idx += 1
...
with col2:
    if not st.session_state.main_data:
        with st.container(border=True):
            st.info("Belum ada data yang ditambahkan. Silakan isi data di sebelah kiri.")
    else:
        with st.container(border=True):
            st.subheader("2. 🎛️ Daftar & Pengaturan Server")
            servers_to_shorten = []
            ...
            for i, s_name in enumerate(server_list):
                ...
                with control_cols[0]:
                    if st.checkbox("Perpendek dengan ouo.io", key=f"shorten_{i}", help=f"Perpendek link untuk {s_name}"):
                        servers_to_shorten.append(s_name)
                with control_cols[1]:
                    btn_cols = st.columns(3)
                    if btn_cols[0].button("↑", key=f"up_{i}", use_container_width=True, help="Naikkan urutan", disabled=(i == 0)):
                        st.session_state.server_order.insert(i - 1, st.session_state.server_order.pop(i)); st.rerun()
                    if btn_cols[1].button("↓", key=f"down_{i}", use_container_width=True, help="Turunkan urutan", disabled=(i == len(server_list) - 1)):
                        st.session_state.server_order.insert(i + 1, st.session_state.server_order.pop(i)); st.rerun()
                    if btn_cols[2].button("⌦", key=f"del_{i}", use_container_width=True, help=f"Hapus server {s_name}"):
                        ...
                        for ep_num, res_data in st.session_state.main_data.items():
                            for res, server_links in res_data.items():
                                if s_name in server_links:
                                    ...
                                    if link_key in st.session_state: st.session_state.main_data[ep_num][res][s_name] = st.session_state[link_key]
                        if new_server_name != s_name and new_server_name:
                            ...
                            for ep_data in st.session_state.main_data.values():
                                for res, res_data_inner in ep_data.items():
                                    if s_name in res_data_inner: res_data_inner[new_server_name] = res_data_inner.pop(s_name)
                        st.success(f"Perubahan untuk server '{s_name}' telah disimpan!"); st.rerun()
                st.divider()

        with st.container(border=True):
            st.subheader("3. 📤 Pilih Format Output")
            output_format = st.radio("Pilih format HTML:", ["Format Drakor", "Format Ringkas"], key="output_format", horizontal=True)
            ...
            if output_format == "Format Drakor":
                ...
                c1, c2 = st.columns(2)
                grouping_style = c1.radio("Urutkan berdasarkan:", ["Server", "Resolusi"], key="grouping_style", horizontal=True)
